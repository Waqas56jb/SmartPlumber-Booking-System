...
const SellerForgotPassword = () => {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const response = await sellerAPI.sellerForgotPassword(seller_email);
      if (response.success) {
        try {
        ...
        background: 'linear-gradient(to bottom, #FEFEFE, #F5E6D3)'
          ...
          <div className="w-full max-w-sm bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-5 md:p-6 lg:p-7 shadow-2xl mt-20 sm:mt-24 lg:mt-0">
            <div className="mb-4 sm:mb-5 md:mb-6 text-center">
              ...
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-1.5 sm:mb-2">Forgot Seller Password?</h2>
              <p className="text-xs sm:text-sm md:text-base text-gray-600">Enter your seller email to receive OTP</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 sm:space-y-4 md:space-y-5">
              ...
              border: '1px solid #D2A752'
                ...
                <p className="text-xs sm:text-sm text-gray-700 text-center leading-relaxed">
                  ...
                  We'll send a verification code to your seller email address to reset your password securely.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" id="seller_email" name="seller_email" value={seller_email} onChange={e => setSellerEmail(e.target.value)} placeholder="Enter your seller email address" className="w-full pl-9 sm:pl-10 md:pl-12 pr-3 sm:pr-4 h-11 sm:h-12 text-sm sm:text-base bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 text-gray-900 placeholder:text-gray-400 shadow-sm" style={{
