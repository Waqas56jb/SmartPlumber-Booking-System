...
const PlumberResetPassword = () => {
  ...
  const handlePasswordSubmit = async e => {
    ...
    try {
      const otpValue = otp.join('');
      const response = await plumberAPI.plumberResetPassword(plumber_email, otpValue, formData.new_plumber_password, formData.confirm_plumber_password);
      if (response.success) {
        try {
          ...
          <div className="w-full max-w-sm bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-5 md:p-6 lg:p-7 shadow-2xl mt-20 sm:mt-24 lg:mt-0 flex flex-col">
            <div className="mb-4 sm:mb-5 md:mb-6 text-center">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600">
                {step === 1 ? `We sent a code to ${plumber_email || 'your plumber email'}` : 'Create your new plumber password'}
              </p>
            </div>
              ...
              background: '#D2A752'
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? 'text' : 'password'} id="new_plumber_password" name="new_plumber_password" value={formData.new_plumber_password} onChange={handlePasswordChange} placeholder="Enter new plumber password" className="w-full pl-9 sm:pl-10 md:pl-12 pr-9 sm:pr-10 md:pr-12 h-11 sm:h-12 text-sm sm:text-base bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 text-gray-900 placeholder:text-gray-400 shadow-sm" style={{
                  '--tw-ring-color': '#D2A752'
                }} onFocus={e => {
                  e.target.style.boxShadow = '0 0 0 2px #D2A752';
                  e.target.style.borderColor = '#D2A752';
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showConfirmPassword ? 'text' : 'password'} id="confirm_plumber_password" name="confirm_plumber_password" value={formData.confirm_plumber_password} onChange={handlePasswordChange} placeholder="Confirm new plumber password" className="w-full pl-9 sm:pl-10 md:pl-12 pr-9 sm:pr-10 md:pr-12 h-11 sm:h-12 text-sm sm:text-base bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 text-gray-900 placeholder:text-gray-400 shadow-sm" style={{
                  '--tw-ring-color': '#D2A752'
                }} onFocus={e => {
                  e.target.style.boxShadow = '0 0 0 2px #D2A752';
                  e.target.style.borderColor = '#D2A752';
                  ...
                  {formData.confirm_plumber_password && formData.new_plumber_password === formData.confirm_plumber_password && <div className="flex items-center gap-2 text-xs sm:text-sm text-green-600 mt-2">
