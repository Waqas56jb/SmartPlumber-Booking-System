      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {isAvailable ? 'You are visible to customers looking for plumbers.' : 'You are hidden from search results.'}
              </p>
            </div>
            <button onClick={() => setIsAvailable(!isAvailable)} className={`w-16 h-8 rounded-full transition-all relative ${isAvailable ? 'bg-green-500' : 'bg-gray-300'}`}>
              <div className={`absolute w-6 h-6 bg-white rounded-full top-1 transition-all shadow-md ${isAvailable ? 'left-9' : 'left-1'}`} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center gap-2 mb-2">
            <FaClock className="text-xl" style={{
            color: '#D2A752'
          }} />
          ...
          <div className="space-y-3">
            ...
            return <div key={day} className={`flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl transition-all ${daySchedule.enabled ? 'bg-gradient-to-r from-[#F5E6D3] to-[#FDF8F3] border-2 border-[#D2A752]/30' : 'bg-gray-50 border-2 border-gray-200'}`}>
                  ...
                  <div className="flex items-center gap-3 min-w-[140px]">
                    <button type="button" onClick={() => toggleDay(dayKey)} className={`w-12 h-6 rounded-full transition-all relative ${daySchedule.enabled ? 'bg-[#D2A752]' : 'bg-gray-300'}`}>
                  ...
                  {daySchedule.enabled ? <div className="flex flex-wrap items-center gap-3 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <input type="time" value={daySchedule.start} onChange={e => updateTime(dayKey, 'start', e.target.value)} className="px-3 py-2 border-2 border-[#D2A752]/30 rounded-lg focus:outline-none focus:border-[#D2A752] bg-white text-gray-900 font-medium" />
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-600 font-medium">To</span>
                        <input type="time" value={daySchedule.end} onChange={e => updateTime(dayKey, 'end', e.target.value)} className="px-3 py-2 border-2 border-[#D2A752]/30 rounded-lg focus:outline-none focus:border-[#D2A752] bg-white text-gray-900 font-medium" />
                      </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => applyPreset('standard')} className="px-4 py-2 rounded-lg text-sm font-semibold border-2 border-[#D2A752] text-[#D2A752] hover:bg-[#D2A752] hover:text-white transition-all">
                Standard 9-5 (Mon-Fri)
              </button>
              <button type="button" onClick={() => applyPreset('extended')} className="px-4 py-2 rounded-lg text-sm font-semibold border-2 border-[#D2A752] text-[#D2A752] hover:bg-[#D2A752] hover:text-white transition-all">
                Extended Hours (Mon-Sat)
              </button>
              <button type="button" onClick={() => applyPreset('emergency')} className="px-4 py-2 rounded-lg text-sm font-semibold border-2 border-green-500 text-green-600 hover:bg-green-500 hover:text-white transition-all">
                24/7 Emergency
              </button>
              <button type="button" onClick={() => applyPreset('clear')} className="px-4 py-2 rounded-lg text-sm font-semibold border-2 border-red-400 text-red-500 hover:bg-red-500 hover:text-white transition-all">
                Clear All
              </button>
            </div>
          </div>
          ...
          {}
          {Object.keys(schedule).some(day => schedule[day]?.enabled) && <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-[#D2A752]/10 to-[#D2A752]/5 border border-[#D2A752]/20">
        ...
        <div className="flex gap-4">
          <button onClick={handleSave} disabled={saving} className="flex-1 px-6 py-4 rounded-xl font-bold text-white transition-all hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg" style={{
          background: '#D2A752'
        }}>
            {saving ? <>
          ...
          <button onClick={() => navigate('/home')} className="px-6 py-4 rounded-xl font-semibold text-gray-700 bg-white border-2 border-gray-300 hover:bg-gray-50 transition-all shadow-lg">
            Cancel
          </button>
        </div>
      </div>
