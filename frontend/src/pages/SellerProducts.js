...
const SellerProducts = () => {
  ...
  const [stats, setStats] = useState({ total: 0, active: 0, outOfStock: 0, totalValue: 0 });

  const categories = [
    { value: 'boiler_installations', label: 'Boiler Installations' },
    { value: 'boiler_repairs', label: 'Boiler Repairs' },
    ...
    { value: 'appliances_installation', label: 'Appliances Installation' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl font-semibold text-white transition-all flex items-center gap-2 hover:shadow-lg"
              ...
              return (
                <div 
                  ...
                  className={`bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all border-2 ${
                    product.stock_quantity <= 0 ? 'border-red-200' : 'border-transparent'
                  }`}
                >
                  ...
                  <div className="h-48 bg-gradient-to-br from-[#F5E6D3] to-[#E8D4B8] relative overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                      ...
                      {product.stock_quantity <= 0 && (
                        <span className="px-3 py-1 rounded-full text-white text-sm font-bold bg-gray-500">
                          Out of Stock
                        </span>
                      )}
                    </div>
                    ...
                    <div className="absolute top-3 right-3">
                      {product.is_active ? (
                        <span className="px-3 py-1 rounded-full text-green-700 text-xs font-semibold bg-green-100 flex items-center gap-1">
                          <FaEye size={10} /> Active
                        </span>
                      ) : (
                        <span className="px-3 py-1 rounded-full text-gray-700 text-xs font-semibold bg-gray-100 flex items-center gap-1">
                  ...
                  <div className="p-5">
                    {/* Category */}
                    <div className="mb-2">
                      <span className="px-3 py-1 rounded-full text-xs font-medium text-white" style={{ background: '#D2A752' }}>
                    ...
                    {(product.product_brand || product.product_model) && (
                      <div className="flex items-center gap-2 text-xs text-gray-400 mb-3">
                        ...
                        {product.product_brand && product.product_model && <span>•</span>}
                    ...
                    <div className="flex flex-wrap gap-2 mb-4 text-xs">
                      ...
                      {product.shipping_charges === 0 || !product.shipping_charges ? (
                        <span className="px-2 py-1 bg-green-50 text-green-600 rounded-lg flex items-center gap-1">
                          <FaTruck size={10} /> Free shipping
                        </span>
                      ) : (
                        <span className="px-2 py-1 bg-gray-50 text-gray-600 rounded-lg flex items-center gap-1">
                      ...
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => (
                          <FaStar
                            key={i}
                            size={14}
                            className={i < Math.floor(parseFloat(product.product_rating) || 0) ? 'text-yellow-400' : 'text-gray-200'}
                          />
                        ))}
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all flex items-center justify-center gap-2 hover:shadow-md"
                      ...
                      <button
                        ...
                        className="px-4 py-2.5 rounded-xl text-sm font-semibold text-red-600 border-2 border-red-200 transition-all hover:bg-red-600 hover:text-white hover:border-red-600"
                      >
                        <FaTrash />
                      </button>
                    </div>
