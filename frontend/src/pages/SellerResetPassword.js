...
const SellerResetPassword = () => {
  ...
  const handlePasswordSubmit = async e => {
    ...
    try {
      const otpValue = otp.join('');
      const response = await sellerAPI.sellerResetPassword(seller_email, otpValue, formData.new_seller_password, formData.confirm_seller_password);
      if (response.success) {
        try {
          ...
          <div className="w-full max-w-sm bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-5 md:p-6 lg:p-7 shadow-2xl mt-20 sm:mt-24 lg:mt-0 flex flex-col">
            <div className="mb-4 sm:mb-5 md:mb-6 text-center">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600">
                {step === 1 ? `We sent a code to ${seller_email || 'your seller email'}` : 'Create your new seller password'}
              </p>
            </div>
            ...
            {step === 1 ? <form onSubmit={handleOtpSubmit} className="space-y-3.5 sm:space-y-4 md:space-y-5 flex-1 flex flex-col">
              ...
              background: '#D2A752'
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? 'text' : 'password'} id="new_seller_password" name="new_seller_password" value={formData.new_seller_password} onChange={handlePasswordChange} placeholder="Enter new seller password" className="w-full pl-9 sm:pl-10 md:pl-12 pr-9 sm:pr-10 md:pr-12 h-11 sm:h-12 text-sm sm:text-base bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 text-gray-900 placeholder:text-gray-400 shadow-sm" style={{
                  '--tw-ring-color': '#D2A752'
                }} onFocus={e => {
                  e.target.style.boxShadow = '0 0 0 2px #D2A752';
                  e.target.style.borderColor = '#D2A752';
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showConfirmPassword ? 'text' : 'password'} id="confirm_seller_password" name="confirm_seller_password" value={formData.confirm_seller_password} onChange={handlePasswordChange} placeholder="Confirm new seller password" className="w-full pl-9 sm:pl-10 md:pl-12 pr-9 sm:pr-10 md:pr-12 h-11 sm:h-12 text-sm sm:text-base bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 text-gray-900 placeholder:text-gray-400 shadow-sm" style={{
                  '--tw-ring-color': '#D2A752'
                }} onFocus={e => {
                  e.target.style.boxShadow = '0 0 0 2px #D2A752';
                  e.target.style.borderColor = '#D2A752';
                  ...
                  {formData.confirm_seller_password && formData.new_seller_password === formData.confirm_seller_password && <div className="flex items-center gap-2 text-xs sm:text-sm text-green-600 mt-2">
              ...
              background: '#D2A752'
                ...
                <div className="text-center pt-4 mt-auto">
                  <Link to="/seller-login" className="text-xs sm:text-sm font-semibold transition-colors inline-flex items-center gap-2" style={{
                color: '#D2A752'
              }} onMouseEnter={e => e.target.style.color = '#B8943F'} onMouseLeave={e => e.target.style.color = '#D2A752'}>
                    <FaArrowLeft size={10} />
