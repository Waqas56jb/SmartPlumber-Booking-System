...
const Portfolio = () => {
  ...
  const handleItemClick = (projectId) => {
    ...
    window.dispatchEvent(new CustomEvent('showLightbox', { detail: projectId }));
  };
      ...
      <div className="max-w-[1400px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 relative z-10 w-full overflow-x-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-6 md:gap-7">
          {initialProjects.map((project, index) => (
            <div
              key={project.id}
              className="relative overflow-hidden cursor-pointer aspect-[4/3] bg-black group rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 animate-fade-in-up hover-lift"
        ...
        <div className="text-center mt-10 sm:mt-14 py-8 sm:py-10">
          <button
            ...
            className={`border-2 border-accent px-8 sm:px-12 py-4 sm:py-4.5 text-sm sm:text-base font-bold tracking-wide uppercase cursor-pointer rounded-xl inline-flex items-center gap-3 transition-all hover:-translate-y-1 hover:shadow-xl ${
              showMore 
          ...
          >
            ...
            <FaChevronDown
              className={`transition-transform duration-300 ${showMore ? 'rotate-180 text-white' : 'text-accent'}`}
            />
          </button>
        </div>

        <div
          className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-6 md:gap-7 transition-all duration-700 overflow-hidden ${
            showMore
              ? 'max-h-[5000px] opacity-100 mt-8 sm:mt-10'
