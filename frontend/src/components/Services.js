      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {services.map((service, index) => (
            <div 
              key={index}
              className="bg-background-alt p-6 sm:p-8 md:p-9 transition-all border-l-[3px] border-transparent hover:bg-white hover:border-l-accent hover:shadow-md hover:translate-x-1"
            >
              <div className="text-xs sm:text-sm font-bold text-accent mb-3 sm:mb-4 tracking-wide">
                {service.number}
              </div>
              <h3 className="text-lg sm:text-xl md:text-[22px] font-bold text-primary mb-2 sm:mb-3">
                {service.title}
              </h3>
