...
const Analytics = ({ apiBase }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [uRes, bRes, tpRes, tprRes] = await Promise.all([
          fetch(`${apiBase}/api/admin/analytics/users-per-week`),
          fetch(`${apiBase}/api/admin/analytics/bookings-per-day`),
          fetch(`${apiBase}/api/admin/analytics/top-plumbers`),
          fetch(`${apiBase}/api/admin/analytics/top-products`)
        ]);
