...
const Plumbers = ({ apiBase }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-slate-300 text-sm">Loading plumbers from database...</div>
      ) : plumbers.length === 0 ? (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {p.is_available ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-emerald-500/15 text-emerald-300">
                          <FiCheckCircle /> Available
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-slate-700 text-slate-200">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <input
                  ...
                  max="5"
                  className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-700 text-slate-100 text-sm"
                  value={editing.plumber_rating || 0}
                  onChange={(e) =>
                    setEditing({ ...editing, plumber_rating: parseFloat(e.target.value) || 0 })
                  }
                />
              </div>
